...
namespace bbelt_exam.Controllers
{
    public class ActivityController : Controller
    {
        ...
        {
            Activity activity = _context.Activities.SingleOrDefault(a => a.ActivityId == activity_id);
        ...
        [HttpGet]
        [Route("Activity/ActivityDetails/{activity_id}")]
        public IActionResult ActivityDetails( int activity_id)
        {
            Activity activity = _context.Activities.Include(a => a.Participants).ThenInclude(p => p.ParticipatingUsers).SingleOrDefault(a => a.ActivityId == activity_id);
            int num = activity.UserId;
            ViewBag.User = _context.Users.SingleOrDefault( u => u.UserId == num);
            ViewBag.ActiveUserId = ActiveUser.UserId;
            return View(activity);
        }
        public HomeModels InitializeDashboard()
        {
            List<Activity> ParticipatingActivities = _context.Participate.Where( p => p.UserId == ActiveUser.UserId).Select(u => u.ParticipatingActivities).ToList();
        ...
        public IActionResult ActivityDetails1(int activity_id)
        {
            Activity activity = _context.Activities.Include(a => a.Participants).ThenInclude(p => p.ParticipatingUsers).SingleOrDefault(a => a.ActivityId == activity_id);
            int num = activity.UserId;
            ViewBag.User = _context.Users.SingleOrDefault(u => u.UserId == num);
